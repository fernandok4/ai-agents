...
When you need to query or inspect data in a database, always use the `/db-query` skill. Do not run raw SQL commands directly via Bash — delegate to `/db-query`, which enforces read-only access and safe query practices.
